...
public enum PlayerAnimation {
    ...
    public Sprite getHatSprite(boolean moving) {
        ...
        if (this == PlayerAnimation.PLAYER_DOWN || this == PlayerAnimation.PLAYER_UP) {
            sprite = new Sprite(Hat.FRONT, -0.5, -2);
        } else {
            sprite = new Sprite(Hat.SIDE, 1, -2);
        }
        if ((moving && frames.get(gunSelected)[currentFrame].lower) || (!moving && defaultFrame.get(gunSelected).lower)) {
            sprite.addXYScaled(0, 1);
        }
        return sprite;
    }
}
